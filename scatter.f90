...
! data files for scattering amplitude and wave function
  open (unit=18, file='NAME_DATE.amp')
  open (unit=19, file='NAME_DATE.wave')
...
! cross1 - total cross section using optical theorem
    cross1=2*dsqrt(2*pi)*imagpart(vec(ngrid+1))
...
40       continue
          ...
          if (ip.eq.1) then
              if (ipd.eq.1) mc(iel,il)=0
              if (ipd.eq.0) mc(iel,il)=ngrid+1

           else if (ip.eq.2*nelp+1) then
              if (ipd.eq.1) mc(iel,il)=0
              if (ipd.eq.0) mc(iel,il)=ngrid+1+(ip-2)*2+1
           else
                mc(iel,il)=ngrid+1+(ip-2)*2+ipd+1
           endif
           go to 50
